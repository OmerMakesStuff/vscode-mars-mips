...
class MipsyCompletionItemProvider implements vscode.CompletionItemProvider {
  ...
  private getInstructionCompletionItems(
    document: vscode.TextDocument,
    ...
    return Object.keys(allInstructions).map((instruction) => {
      return {
        ...
        insertText:
          instruction +
          (autoIndent && !['syscall', 'begin', 'end'].includes(instruction)
            ? '\t'
            : ''),
  ...
  private getDirectiveCompletionItems(
    ...
    return Object.keys(directives).map((directive) => {
      return {
        ...
        sortText: prependSymbol
          ? this.sortOrders.directive.toString()
          : '.' + directive,
      };
    });
  }
  ...
  private getRegisterCompletionItems(
    ...
    return Object.keys(registers).map((register) => {
      return {
        ...
        sortText: prependSymbol
          ? this.sortOrders.register.toString()
          : '$' + register,
      };
    });
  }

  private getLabelCompletionItems(
    document: vscode.TextDocument,
  ): vscode.CompletionItem[] {
    const items = [];
  ...
  private getConstantCompletionItems(
    document: vscode.TextDocument,
  ): vscode.CompletionItem[] {
    const items = [];
  ...
  private getSnippetCompletionItems(
    ...
    function requiredCommentIndent(
      ...
      const requiredTabCount = Math.ceil(
        (targetColumn - currentColumn) / tabSize,
      );
    ...
    let commentColumn =
      vscode.workspace.getConfiguration('mars-mips', document)
        .snippetCommentColumn ?? 32;
    ...
    return [
      ...
      },
      {
        ...
        insertText: new vscode.SnippetString()
          ...
          .appendText('la\t$a0, ')
          .appendChoice(
            getLabelDefinitions(document).map((label) => label.name),
          )
          .appendText('\n')
          .appendText((text = 'syscall'))
          .appendText(requiredCommentIndent(text, tabSize, commentColumn))
      ...
      },
      {
        ...
        insertText: new vscode.SnippetString()
          ...
          .appendText((text2 = ', $v0'))
          .appendText(
            requiredCommentIndent(text + text2 + 't0', tabSize, commentColumn),
          )
      ...
      },
      {
        ...
        insertText: new vscode.SnippetString()
          ...
          .appendText('li\t$a1, ')
          .appendChoice(
            getConstantDefinitions(document).map((constant) => constant.name),
          )
          .appendText('\n')
          .appendText((text = 'syscall'))
          .appendText(requiredCommentIndent(text, tabSize, commentColumn))
