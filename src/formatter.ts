...
export default function formatter(document: TextDocument): TextEdit[] {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    if (line.label && !line.directive && firstLabel === null)
      firstLabel = index;
  ...
  const edits = lines.map((line) => {
    ...
    if (
      !line.label &&
      line.directive &&
      !['.data', '.text', '.macro', '.end_macro'].includes(line.directive)
    )
      result = result.padEnd(longestLabel, ' ');
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!line.label && !line.directive && !line.instruction)
      edits[i] = TAB + line.comment;
    else if (line.label && line.directive && line.instruction)
      edits[i] += ` ${line.comment}`;
  ...
  return [
    TextEdit.replace(range, edits.map((edit) => edit.trimEnd()).join('\n')),
  ];
}
